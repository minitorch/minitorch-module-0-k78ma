...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    # Check if sigmoid is always between 0.0 and 1.0
    assert 0.0 <= sigmoid(a) <= 1.0

    # Check if one minus sigmoid is the same as sigmoid of the negative
    assert_close(1 - sigmoid(a), sigmoid(-a))

    # Check if it crosses 0 at 0.5
    assert_close(sigmoid(0.0), 0.5)

    # Check if it is strictly increasing
    if a < 0:
        assert sigmoid(a) <= sigmoid(0)
    elif a > 0:
        assert sigmoid(0) <= sigmoid(a)
